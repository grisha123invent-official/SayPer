...
Приложение здесь целиком — 45 файлов Swift, без зависимостей и пакетных менеджеров. Клонируешь, запускаешь `./build.sh install`, и через минуту у тебя своя копия. Меняй поведение хоткея, ставь вместо Whisper локальную модель, перекраивай окно — это твоё дело.
