...
enum OutputDucker {
    ...
    static func duck() {
        ...
        // Само приглушение осталось прежним, добавлено только решение
        // «надо ли» — см. `AudioDevices.shouldDuck()`.
        guard AudioDevices.mayTouchOutput() else {
            Log.write("Приглушение пропущено: пишем встроенным, а рядом гарнитура — "
                      + "в микрофон из неё ничего не попадает")
            return
        }
