...
enum OutputDucker {
    ...
    static func duck() {
        ...
        // Беспроводную гарнитуру не трогаем. Одной записи громкости хватает,
        // чтобы наушники, подключённые и к маку, и к телефону, перескочили
        // на мак и оборвали музыку на телефоне.
        //
        // Потери при этом нет: приглушение существует, чтобы звук компьютера
        // не лез в микрофон. Из наушников он туда не лезет — они на голове,
        // а пишем мы со встроенного микрофона. Глушить есть смысл только
        // динамики, и именно они под это условие подходят.
        guard AudioDevices.mayTouchOutput() else {
            Log.write("Приглушение пропущено: вывод на беспроводной гарнитуре, "
                      + "которую не выбрали микрофоном")
            return
        }
