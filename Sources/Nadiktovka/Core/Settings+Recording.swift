...
extension Settings {
    ...
    ///
    /// Пишут это свойство двое — карточка «Режим» и пункт меню (тот через
    /// `set(_:forKey:)` с тем же ключом). Читает `RecordingGate`, он же следит
    /// за изменением: перезапуск для смены режима не нужен.
